...
int readEventsReco(TString list = "data/filesReco_SimCampaign.list", TString ofname = "output/output_test.root", long nevents = -1);
...
int readEventsReco(TString list, TString ofname, long nevents)
{
	...
	for(unsigned ev=0; ev<nEvents; ++ev) {
		...
		MakeEvent(store, ev_all); // execute for each event
...
int MakeEvent(podio::EventStore *store, unsigned ev)
{
	...
	auto& nHCal_hits_frame  = store->get<edm4eic::CalorimeterHitCollection>("HcalEndcapNRecHits");
	auto& MCParticles_frame  = store->get<edm4hep::MCParticleCollection>("MCParticles");
	//store->get<edm4hep::MCParticleCollection>("MCParticles");
	//const edm4hep::MCParticleCollection MCParticles_frame  = store->get<edm4hep::MCParticleCollection>("MCParticles");
	...
	if(!nHCal_hits_frame.isValid())
		cout<<"CalorimeterHitCollection does not exist!"<<endl;

	if(debug) cout<<"CalorimeterHitCollection size = "<<nHCal_hits_frame.size()<<endl;
